...
# ---------PICK WEAPONS
        ...
        while pick_weapons:
            ...
            # QUITAR:
            elif num_negativo == True and int(select_weapon) not in new_ch_weapons:
                print("Invalid Option".center(40, "="))
                input("Press enter to continue\n")
...
# ----------SAVE CHARACTER
        print()
        # print para cuando lista de armas de pj == 0
        if len(new_ch_weapons) == 0:
            print(f"The new player will be:\nName: {new_ch_name}\n"
                  f"Category: {dict_categorys[new_ch_category]}\n"
                  f"Weapons: \n"
                  f"Strength: {new_ch_strength}\n"
                  f"Speed: {new_ch_speed}\n"
                  f"Experience: {new_ch_experience}")
        # print para cuando lista de armas de pj == 1
        elif len(new_ch_weapons) == 1:
            print(f"The new player will be:\nName: {new_ch_name}\n"
                  f"Category: {dict_categorys[new_ch_category]}\n"
                  f"Weapons: {dict_weapons[new_ch_weapons[0]]['name']}\n"
                  f"Strength: {new_ch_strength}\n"
                  f"Speed: {new_ch_speed}\n"
                  f"Experience: {new_ch_experience}")
        # print para cuando lista de armas de pj == 2
        elif len(new_ch_weapons) == 2:
            print(f"The new player will be:\nName: {new_ch_name}\n"
                  f"Category: {dict_categorys[new_ch_category]}\n"
                  f"Weapons: {dict_weapons[new_ch_weapons[0]]['name']}, {dict_weapons[new_ch_weapons[1]]['name']}\n"
                  f"Strength: {new_ch_strength}\nSpeed: {new_ch_speed}\n"
                  f"Experience: {new_ch_experience}")

        confirm = input("Save this player? Y/N: ")
        confirm = confirm.upper()
        while not confirm.isalpha() or not confirm in ("Y", "N"):
            print("Invalid Option".center(40, "="))
            confirm = input("Save this player? Y/N: ")
            confirm = confirm.upper()
...
# ------------SAVE WEAPON
        print(f"The new weapon will be:\n"
              f"Name: {new_weapon_name}\n"
              f"Strength: {new_weapon_strength}\n"
              f"Speed: {new_weapon_speed}\n"
              f"Two hands type: {new_weapon_kind}\n")

        confirm = input("Save this weapon? Y/N: ")
        confirm = confirm.upper()
        while not confirm.isalpha() or not confirm in ("Y", "N"):
            print("Invalid Option".center(40, "="))
            confirm = input("Save this weapon? Y/N: ")
            ...
            if opc==1:
                ...
                for clave in dict_characters:
                    ...
                    if len(dict_characters[clave]['weapons']) == 0:
                        print(f"ID: {clave}, Name: {dict_characters[clave]['name']}, Weapons: None"
                    ...
                    # print para cuando lista de armas de pj == 1
                        ...
                        print(f"ID: {clave}, Name: {dict_characters[clave]['name']}"
                              f", Weapons: {dict_weapons[dict_characters[clave]['weapons'][0]]['name']}"
                            f", Strenght: {dict_characters[clave]['strength']}"
                            f", Speed: {dict_characters[clave]['speed']}"
                            f", Experience: {dict_characters[clave]['experience']}")
                    ...
                    # print para cuando lista de armas de pj == 2
                        ...
                        print(f"ID: {clave}, Name: {dict_characters[clave]['name']}"
                              f", Weapons: {dict_weapons[dict_characters[clave]['weapons'][0]]['name']}, {dict_weapons[dict_characters[clave]['weapons'][1]]['name']}"
                            f", Strenght: {dict_characters[clave]['strength']}"
                            f", Speed: {dict_characters[clave]['speed'] }"
                            f", Experience: {dict_characters[clave]['experience']}")
                                ...
                                if (len(new_ch_weapons)) == 2:
                                    ...
                                    for i in range(1, len(dict_weapons) + 1):
                                        if len(new_ch_weapons) > 0 and dict_weapons[i]['two_hand'] == True:
                                            ...
                                            print(f"{i}) {dict_weapons[i]['name']}, "
                                                  f"Strength: {dict_weapons[i]['strength']}, "
                                                  f"Speed: {dict_weapons[i]['speed']}")
                                    print()
                                # CHARACTER WEAPONS:
                                print("Character Weapons".center(40, "="))
                                if new_ch_weapons == []:
                                    ...
                                    for clave in new_ch_weapons:
                                        if clave in dict_weapons:
                                            print(f"{clave}) {dict_weapons[clave]['name']}, "
                                                  f"Strength: {dict_weapons[clave]['strength']}, "
                                                  f"Speed: {dict_weapons[clave]['speed']}")
                                    print()
                                # ELEGIR OPCION:
                                select_weapon = input("Add Weapons:\nWeapon Id) To add weapon ID\n0) Finish add weapons\n-Weapon Id) Delete Weapon Id\n->Option: ")
                                num_negativo = False
                ...
                while not id_value.isdigit() or int(id_value) not in range(1, len(dict_weapons) + 1):
                    ...
                    while not opc.isdigit() or int(opc) not in (1, 2, 3, 4):
                        ...
                        # NEW WEAPON STRENGTH
                            ...
                            value = input("Enter the new Strength 1-9: ")
                            value = int(value)
                        ...
                        # NEW WEAPON SPEED
                            ...
                            value = input("Enter the new Speed 1-9: ")
                            value = int(value)
                            while value not in range(1, 10):
                                print("Invalid Option".center(40, "="))
                                input("Press enter to continue\n")
...
#/////////FLG MENU 041 LIST CHARACTERS
    while flg_menu_041:
        menu_041 = "Menu 042 (List Characters)".center(40,"=") + "\n1)List by ID\n2)List by name\n3)List by strength\n4)List by speed\n5)Go back"
        print(menu_041)
        opc = input("->Option: ")
        while not opc.isdigit() or int(opc) not in range (1,6):
            print("Invalid Option".center(40, "="))
            input("Press enter to continue\n")
            print(menu_04)
            opc = input("->Option: ")
...
# -----------LIST CHARACTERS BY ID
            if opc == 1:
                print("Characters ordered by ID".center(60, "=") + "\n" + "Id".ljust(10) + "Name".ljust(17) +
                      "Strength".rjust(8) + " " * 5 + "Speed".rjust(5) + " " * 5 +
                      "Experience".ljust(10) + "\n" + "*" * 60)
                for clave in dict_characters:
                    if len(dict_characters[clave]['weapons']) == 0:
...
# -----------LIST CHARACTERS BY STRENGTH
                ...
                for pasada in range(len(claves_characters) - 1):
                    for j in range(len(claves_characters) - 1 - pasada):
                        # suma_p1 es la suma de fuerza base del pj + fuerza de las armas de: j
                        suma_p1 = 0
                        claves_wp = list(dict_weapons.keys())
                        # Itero sobre la longitud de la lista de armas añadidas en el pj
                        for i in range(len(dict_characters[claves_characters[j]]['weapons'])):
                            # Itero por cada arma, dict_weapons para comprobar qué id coincide
                            for arma in range(len(dict_weapons)):
                                # Si es el mismo id, añade en suma la fuerza del arma
                        ...
                        # suma_p2 es la suma de fuerza base del pj + fuerza de las armas de: j+1
                        suma_p2 = 0
                        claves_wp = list(dict_weapons.keys())
                        # Itero sobre la longitud de la lista de armas añadidas en el pj
                        ...
                        # suma_p1 = fuerza_base_pj + fuerza_armaN
                        # suma_p2 = fuerza_base_pj + fuerza_armaN
                        if suma_p1 > suma_p2:
                            aux = claves_characters[j]
                            claves_characters[j] = claves_characters[j + 1]
                            claves_characters[j + 1] = aux
                for clave in claves_characters:
                    # print para cuando hayan 0 armas en lista de "weapons"
                    if len(dict_characters[clave]['weapons']) == 0:
                        print(f"{clave}".ljust(10)
                              + f"{dict_characters[clave]['name']}".ljust(17)
                              + f"{dict_characters[clave]['strength']}".rjust(8) + " "*5
                    ...
                    # print para cuando haya 1 arma en lista de "weapons"
                    elif len(dict_characters[clave]['weapons']) == 1:
                        print(f"{clave}".ljust(10)
                              + f"{dict_characters[clave]['name']}".ljust(17)
                              + f"{dict_characters[clave]['strength'] + dict_weapons[dict_characters[clave]['weapons'][0]]['strength']}".rjust(8) + " "*5
                    ...
                    # print para cuando hayan 2 armas en lista de "weapons"
                    elif len(dict_characters[clave]['weapons']) == 2:
                        print(f"{clave}".ljust(10)
                              + f"{dict_characters[clave]['name']}".ljust(17)
                              + f"{dict_characters[clave]['strength'] + dict_weapons[dict_characters[clave]['weapons'][0]]['strength'] + + dict_weapons[dict_characters[clave]['weapons'][1]]['strength']}".rjust(8) + " " * 5
...
# -----------LIST WEAPONS BY ID
            if opc == 1:
                ...
                for clave in dict_weapons:
                    print(str(clave).ljust(10) + dict_weapons[clave]['name'].ljust(17) +
                          str(dict_weapons[clave]['strength']).rjust(8) + " " * 5 +
                          str(dict_weapons[clave]['speed']).rjust(5) + " " * 5 + str(dict_weapons[clave]['two_hand']).ljust(10)
                          )
            ...
            for i in range(len(dict_characters[clave]['weapons'])):
                for arma in range(len(dict_weapons)):
                    if dict_characters[clave]['weapons'][i] == claves_wp[arma]:
                        speed += dict_weapons[claves_wp[arma]]['speed']
            speed += dict_characters[clave]['speed']
            print(str(clave).ljust(10) + dict_characters[clave]['name'].ljust(17) + str(strength).rjust(8) + " " * 5 +
                  str(speed).rjust(5) + " " * 5 + str(dict_characters[clave]['experience']).rjust(10) +
                  dict_categorys[dict_characters[clave]['category']].rjust(20))
